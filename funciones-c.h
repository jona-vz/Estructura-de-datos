...
void CambiarPrioridad(int NumElem,int destino,Cola* cola){
  Nodo *temp, *temp2, *aux;
  temp = cola->first;
  int i;
...
void Cambiardecola(Cola *origen,Cola *destino){
	
	Insertar(destino,origen->first->dato);
	printf("estoy en cambiar de cola");
	Eliminar(origen);
